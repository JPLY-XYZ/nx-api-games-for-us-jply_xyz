...
export async function POST(request) {
    try {
        ...
        let {usuarioId, fullName} = decryptJSON(body.encryptedData, body.iv, process.env.ENCRYPTION_CLIENT_KEY);
        ...
        if (usuario.fullName == fullName) {
            return Response.json(
                { message: "El fullName es el mismo" },
                { status: 401 }
            );
        }
